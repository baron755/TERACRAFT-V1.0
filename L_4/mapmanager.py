...
class Mapmanager():
    def __init__(self):
        ...
        textures = [
            ...
            'block/bylurnuc.png',
            'block/leaf.png',
            'block/black_stone.png',
            'block/Red.png',
            'block/Blue.png',
            'block/Green.png',
            'block/not_ricroll.jpg'
    ...
    def save_map_2(self):
        blocks = self.land.getChildren()
        with open("my_save_2.dat", "wb") as fout:
            pickle.dump(len(blocks), fout)
            for block in blocks:
                x,y,z = block.getPos()
                pos = (int(x),int(y),int(z))
    ...
    def load_map_2(self):
        self.clear()
        with open("my_save_2.dat", "rb") as fin:
            length = pickle.load(fin)
            for _ in range(length):
                pos = pickle.load(fin)
                type = pickle.load(fin)
    ...
    def save_map_3(self):
        blocks = self.land.getChildren()
        with open("my_save_3.dat", "wb") as fout:
            pickle.dump(len(blocks), fout)
            for block in blocks:
                x,y,z = block.getPos()
                pos = (int(x),int(y),int(z))
    ...
    def load_map_3(self):
        self.clear()
        with open("my_save_3.dat", "rb") as fin:
            length = pickle.load(fin)
            for _ in range(length):
                pos = pickle.load(fin)
                type = pickle.load(fin)
